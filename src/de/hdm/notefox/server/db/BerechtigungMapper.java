...
public class BerechtigungMapper {
	...
	public Berechtigung nachBerechtigungIdSuchen(int id) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT berechtigungId, berechtigungsart  FROM berechtigung "
			...
			if (rs.next()) {
				...
				be.setBerechtigungsart(Berechtigungsart.valueOf(rs.getString("berechtigungsart")));
	...
	public List<Berechtigung> nachAllenBerechtigungenDerNotizobjekteSuchen(BusinessObject notizobjekt) {
		return nachAllenBerechtigungenDerNotizobjekteSuchen(notizobjekt, null);
	}

	public List<Berechtigung> nachAllenBerechtigungenDerNotizobjekteSuchen(BusinessObject notizobjekt, Nutzer nutzer) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT berechtigung.*, nutzer.* FROM berechtigung LEFT JOIN nutzer ON nutzer.nutzerId = berechtigung.berechtigter"
							+ " WHERE " + ((notizobjekt instanceof Notiz) ? "notiz" : "notizbuch") + "=" + id + " "
							+ (nutzer != null ? "AND berechtigter =" + nutzer.getNutzerId() : ""));
			...
			while (rs.next()) {
				...
				be.setBerechtigungsart(Berechtigungsart.valueOf(rs.getString("berechtigung.berechtigungsart")));
				result.add(be);
			}
	...
	public Berechtigung anlegenBerechtigung(Berechtigung be) {
		...
		try {
			 ...
			 */
			ResultSet rs = stmt.executeQuery("SELECT MAX(berechtigungId) AS maxid " + "FROM berechtigung ");
			...
			if (rs.next()) {
				...
				String sql = "INSERT INTO berechtigung (berechtigungId, berechtigungsart, notiz, notizbuch, berechtigter ) "
						+ "VALUES (" + be.getBrechtigungId() + ", \"" + be.getBerechtigungsart() + "\", "
	...
	public boolean exisitiertBerechtigung(Berechtigung berechtigung) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM berechtigung WHERE "
					+ (berechtigung.getNotiz() != null ? "notiz=" + berechtigung.getNotiz().getId() : "notiz IS NULL")
					+ " AND "
					+ (berechtigung.getNotizbuch() != null ? "notizbuch=" + berechtigung.getNotizbuch().getId()
							: "notizbuch IS NULL")
					+ " AND berechtigungsart = \"" + berechtigung.getBerechtigungsart() + "\" AND berechtigter = "
